...
1. Ask Claude to generate today's exam ("give me today's mock exam", "/mock", "start a mock exam").
   Claude authors/selects 40 questions from `bank.json` (adding new ones grounded in the syllabus
...
4. Answer questions in any order using the question navigator (left sidebar). Flag any question
   for later review. Progress (answers, flags, current question, remaining time) is saved to the
...
1. Read `bank.json` to see what already exists (avoid near-duplicate questions) and read any
   files in `question-bank/results/` to see which topics have been getting missed.
...
3. Author new questions (or targeted variants of weak-topic questions using different scenarios
   and wording — never the exact same question repeated) following the question quality bar in
   `prompts/question-coach.md`, and append them to `bank.json` with fresh, never-reused IDs.
...
| Command | Effect |
|---|---|
| `python tools/render_exam.py new` | Select today's exam from `bank.json` (default 40 Q / 75 min / 65% pass mark), write a session JSON, render its HTML |
