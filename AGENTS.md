...
It is acceptable to test the same concept again, but the new item must use a fresh stem, scenario, wording, data values, and distractor design. Prefer changing the domain/context (for example banking vs e-commerce), the practical task being asked, and the tempting wrong options. Questions from the supplementary book may inspire coverage or style, but must be rewritten into original CTFL v4.0.1 variants before being saved or asked.
...
Do not rely only on direct definitions, and do not repeat earlier saved or asked questions. If a concept needs more practice, write a new scenario or framing for it.
...
1. Read `prompts/question-coach.md` and the chapter's existing `questions.md` (to avoid repeating identical or near-duplicate questions).
...
2. Author enough new questions (or targeted fresh variants of weak topics) following the question
   ...
   fresh IDs — never delete or overwrite existing entries. New mock sittings must use question
   IDs that have not appeared in previous `question-bank/exams/*.json` sittings; if the bank is
   short, add new variant questions before rendering instead of reusing old ones.
...
`syllabus/progress.md`), mixing topics, using fresh questions rather than repeats from existing
chapter practice, and without revealing answers during the exam. After the user submits all
answers, score it, explain every answer, identify mistake patterns, and propose a personalized
revision plan. Save the results as a new dated section in `revision/final-revision.md`, then run
